...
require_once( plugin_dir_path( __FILE__ ) . '/includes/settings/wsl.database.php'             ); // Install/Uninstall WSL database tables
require_once( plugin_dir_path( __FILE__ ) . '/includes/settings/wsl.initialization.php'       ); // Check WSL requirements and register WSL settings
require_once( plugin_dir_path( __FILE__ ) . '/includes/settings/wsl.compatibilities.php'      ); // Check and upgrade WSL database/settings (for older versions)

# Services & Utilities
require_once( plugin_dir_path( __FILE__ ) . '/includes/services/wsl.authentication.php'       ); // Authenticate users via social networks. <- that's the most important script
require_once( plugin_dir_path( __FILE__ ) . '/includes/services/wsl.mail.notification.php'    ); // Emails and notifications
require_once( plugin_dir_path( __FILE__ ) . '/includes/services/wsl.user.avatar.php'          ); // Display users avatar
require_once( plugin_dir_path( __FILE__ ) . '/includes/services/wsl.user.data.php'            ); // User data functions (database related)
require_once( plugin_dir_path( __FILE__ ) . '/includes/services/wsl.utilities.php'            ); // Unclassified functions & utilities
require_once( plugin_dir_path( __FILE__ ) . '/includes/services/wsl.watchdog.php'             ); // WSL logging agent

# WSL Widgets & Front-end interfaces 
require_once( plugin_dir_path( __FILE__ ) . '/includes/widgets/wsl.auth.widgets.php'          ); // Authentication widget generators (where WSL widget/icons are displayed)
require_once( plugin_dir_path( __FILE__ ) . '/includes/widgets/wsl.complete.registration.php' ); // Force users to complete their profile after they register
require_once( plugin_dir_path( __FILE__ ) . '/includes/widgets/wsl.users.gateway.php'         ); // Planned for WSL 2.3. Accounts linking + Profile Completion
require_once( plugin_dir_path( __FILE__ ) . '/includes/widgets/wsl.error.pages.php'           ); // Generate WSL notices end errors pages
require_once( plugin_dir_path( __FILE__ ) . '/includes/widgets/wsl.loading.screens.php'       ); // Generate WSL loading screens
